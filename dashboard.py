...
def render_dashboard(resumo, horario, p):
    ...
    # KPIs Estilo SOLENERGE (Pág. 3 do Relatório)
    ...
    fig.add_trace(go.Bar(x=resumo['mes'], y=resumo['prod_ac_kwh'], name='Produção (PV)', marker_color='#3B82F6', width=0.4))
    fig.update_layout(barmode='overlay', xaxis_title="Mês", yaxis_title="Energia (kWh)", legend=dict(orientation="h", y=1.1))
    st.plotly_chart(fig, use_container_width=True)

    # Análise Sazonal: Dia Médio Sobreposto
    ...
    def plot_seasonal(df_filtered, title):
        ...
        # Estado de Carga (Eixo Secundário)
        f.add_trace(go.Scatter(x=df_h['hora'], y=df_h['soc_kwh'], name='Bateria (SoC)', line_color='#10B981', yaxis="y2"))
        
        f.update_layout(title=title, xaxis_title="Hora", yaxis_title="Potência (kW)", 
                        yaxis2=dict(title="SoC (kWh)", overlaying="y", side="right"),
                        legend=dict(orientation="h", y=-0.2), height=400)
        return f

    with cv:
        st.plotly_chart(plot_seasonal(horario[horario['mes'] == 6], "Junho (Pico Solar)"), use_container_width=True)
    with ci:
        st.plotly_chart(plot_seasonal(horario[horario['mes'] == 12], "Dezembro (Baixa Irradiância)"), use_container_width=True)
