...
def render_sidebar():
    with st.sidebar:
        ...
        # Parâmetro de Bateria (Conforme Relatório)
        cap_bateria = st.number_input("Capacidade Bateria (kWh)", min_value=0.0, value=100.0, step=5.0) [cite: 59]
        
        st.subheader("☀️ Painéis Fotovoltaicos")
        n_modulos = st.number_input("Nº Módulos", min_value=1, value=180) [cite: 58]
        p_wp = st.number_input("Wp Unitário", value=590) [cite: 58]
        
        p_dc_total = (n_modulos * p_wp) / 1000
        racio = p_dc_total / p_ac_total
        
        st.metric("Rácio DC/AC", f"{racio:.2f}")
        # Recomendação SOLENERGE para Açores 
        if 1.3 <= racio <= 1.4:
        ...
        with st.expander("Geometria e Custos"):
            ...
            inclination = st.slider("Inclinação (°)", 0, 90, 15) [cite: 55]
            azimuth = st.slider("Azimute (°)", 0, 360, 180) [cite: 57]
            consumo_anual = st.number_input("Consumo Anual (kWh)", value=143254) [cite: 54]
            preco_compra = st.number_input("Compra (€/kWh)", value=0.183) [cite: 98]
